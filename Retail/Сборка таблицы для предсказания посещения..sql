...
SELECT  CAST([PurchaseDateTime] as DATE) [PurchaseDate]
      ...
      ,[GroupID]
      ,[CollectionID]
      ,IIF([WearType] = 'КОЛЬЦО',1,0) Kolco
	  ,IIF([WearType] = 'СЕРЬГИ',1,0) SERGI
	  ,IIF([WearType] = 'ПОДВЕС',1,0) PODVES
	  ,IIF([WearType] = 'ЦЕПЬ',1,0) CEP
	  ,IIF([WearType] = 'БРАСЛЕТ',1,0) BRASLET
	  ,IIF([WearType] = 'КОЛЬЕ',1,0) KOLE
	  ,IIF([WearType] = 'БУСЫ',1,0) BUSI
	  ,[WearType]
	  ...
	  ,[MaterialStatusID]
      ,IIF([PriceSegment] = 'ДЕШЕВЫЙ',1,0) Cheap
	  ,IIF([PriceSegment] = 'ДОРОГОЙ',1,0) Expensive
	  ,IIF([PriceSegment] = 'ПРЕМИУМ',1,0) Premium
	  ,IIF([PriceSegment] = 'СРЕДНИЙ',1,0) Middle
	  ,[PriceSegment]
      ,IIF([Metal] = 'ЗОЛОТО',1,0) Zoloto
	  ,IIF([Metal] = 'СЕРЕБРО',1,0) Serebro
	  ,[Metal]
      ,IIF([MetalColor] = 'ЧЕРНЫЙ',1,0) ColorBlack
	  ,IIF([MetalColor] = 'КРАСНЫЙ',1,0) ColorRed
	  ,IIF([MetalColor] = 'ЖЕЛТЫЙ',1,0) ColorYellow
	  ,IIF([MetalColor] = 'БЕЛЫЙ',1,0) ColorWhite
	  ,IIF([MetalColor] = 'БЕЗ МЕТАЛЛА',1,0) ColorBez
	  ,IIF([MetalColor] = '3ХЦВЕТН',1,0) Color3
	  ,IIF([MetalColor] = '2ХЦВЕТН',1,0) Color2
	  ,[MetalColor]
      ,IIF([StylisticGroup] = 'БАЗА',1,0) GroupBaza
	  ,IIF([StylisticGroup] = 'КЛАССИКА',1,0) GroupClassic
	  ,IIF([StylisticGroup] = 'МОДА',1,0) GroupModa
	  ,IIF([StylisticGroup] = 'МОДУЛЬ',1,0) GroupModul
	  ,IIF([StylisticGroup] = 'СУВЕНИРЫ',1,0) GroupSouvenir
	  ,IIF([StylisticGroup] = 'ТРЕНД',1,0) GroupTrend
	  ,[StylisticGroup]
	  ,Ins
	  ,IIF(Ins Is NULL,1,0) NoIns
	  ,IIF(Ins  = 'ФИАНИТ' ,1,0) Fianit
	  ,IIF(Ins  = 'БРИЛЛИАНТ' ,1,0) Diamond
	  ,IIF(Ins  = 'СТЕКЛО' ,1,0) Glass
	  ,IIF(Ins  = 'ТОПАЗ' ,1,0) Topaz
	  ,IIF(Ins  = 'ЖЕМЧУГ' ,1,0) Pearl
	  ,IIF(Ins  = 'ПЛАСТИК' ,1,0) Plastik
	  ,IIF(Ins  = 'ГРАНАТ' ,1,0) Granat
	  ,IIF(Ins  = 'ЦИРКОН' ,1,0) Cirkon
	  ,IIF(Ins  = 'СВАРОВСКИ' ,1,0) Swarovski
	  ,IIF(Ins  = 'САПФИР' ,1,0) Sapfir
	  ,b.Type
	  ,IIF(b.Type  = '2=1' ,1,0) DvaRavnoOdin
	  ,IIF(b.Type  = 'Скидка' ,1,0) Discount
	  ,IIF(b.Type  = 'Скидка+Баллы' ,1,0) Points
	  ,IIF(b.Type  = 'ПД' ,1,0) Sertifikat
	  ,IIF(b.Type  = 'Распродажа' ,1,0) SellOut
	  ,IIF(b.Type  = 'ШОК' ,1,0) Shock
	  into TempPurch
...
--drop table PurchaseGrid
  SELECT [ind],[Period],Month([Period]) Month
      ...
      ,MAX([SMS_1]) [SMS_1] --если был одиин визит по смс, то дальше всегда будет 1
